...
tf.app.flags.DEFINE_string("data_dir", "./podcast_data", "Data directory")
tf.app.flags.DEFINE_string("test_file", "data/podcast_transcripts0qU3MdkMnHLCLdRLg3X5Ha.txt", "Test filename.")
...
tf.app.flags.DEFINE_boolean("geneos", False, "Do not generate EOS. ")
tf.app.flags.DEFINE_float(
    "max_gradient", 1.0, "Clip gradients l2 norm to this range.")
tf.app.flags.DEFINE_integer(
    "batch_size", 1, "Batch size in training / beam size in testing.")
tf.app.flags.DEFINE_integer(
...
def create_model(session, forward_only):
    ...
    print(ckpt)
    print(tf.train.checkpoint_exists(ckpt))
    print('------------------')
...
def train():
    ...
    docid, sumid, doc_dict, sum_dict = \
        data_util.load_data(
            FLAGS.data_dir + "/training_transcripts.txt",
            FLAGS.data_dir + "/train.summaries.txt",
    ...
    val_docid, val_sumid = \
        data_util.load_valid_data(
            FLAGS.data_dir + "/validation_transcripts.txt",
            FLAGS.data_dir + "/validation_summaries.txt",
            doc_dict, sum_dict)
